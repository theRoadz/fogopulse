/**
 * FogoPulse Trade Bot Wallet Setup (Transfer Mode)
 *
 * Generates bot wallet keypairs and funds them with SOL + USDC for trade simulation.
 * Transfers USDC from the master wallet's balance (does NOT require mint authority).
 * Use this on servers where the master wallet is not the USDC mint authority.
 *
 * Usage:
 *   npx tsx setup-fund-trade-bots.ts --count 5
 *   npx tsx setup-fund-trade-bots.ts --count 3 --sol-per-bot 0.2 --usdc-per-bot 200
 *   npx tsx setup-fund-trade-bots.ts --count 5 --wallets-dir ./my-wallets
 *
 * Environment:
...
async function main() {
  ...
  console.log('FogoPulse Trade Bot Setup (Transfer Mode)')
  console.log('='.repeat(60))
  console.log()
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--count' && args[i + 1]) {
      ...
      console.log('Usage: npx tsx setup-fund-trade-bots.ts [options]')
      console.log()
      console.log('Options:')
      console.log('  --count N           Number of bot wallets (default: 5)')
      console.log('  --sol-per-bot X     SOL to fund each bot (default: 0.1)')
      console.log('  --usdc-per-bot Y    USDC to transfer to each bot (default: 100000)')
  ...
  // Check master wallet SOL balance
  ...
  const masterUsdcAta = await getOrCreateAssociatedTokenAccount(
    connection,
    masterWallet,
    USDC_MINT,
    masterWallet.publicKey,
  )
  const masterUsdcBalance = Number(masterUsdcAta.amount) / 10 ** USDC_DECIMALS
  const requiredUsdc = count * usdcPerBot
  console.log(`Master USDC balance: ${masterUsdcBalance.toFixed(2)} USDC`)
  console.log(`Required USDC: ${requiredUsdc.toFixed(2)} USDC (${count} bots × ${usdcPerBot} USDC)`)

  if (masterUsdcBalance < requiredUsdc) {
    console.error(`ERROR: Insufficient USDC. Need at least ${requiredUsdc.toFixed(2)} USDC, have ${masterUsdcBalance.toFixed(2)}.`)
    process.exit(1)
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      ...
      await transfer(
        connection,
        masterWallet,
        masterUsdcAta.address,
        botUsdcAta.address,
        masterWallet,
        usdcLamports,
      )
      console.log(`[Bot-${i}] Transferred ${usdcPerBot} USDC`)
      ...
      console.error(`[Bot-${i}] USDC transfer failed: ${err.message}`)
    }
